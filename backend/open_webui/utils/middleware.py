            ...
            async def tool_call_handler(tool_call):
                ...
                try:
                    ...
                    if tool.get('direct', False):
                        if not callable(event_caller):
                            raise RuntimeError(
                                'Direct tool execution requires an active event caller (WebSocket session).'
                            )
                        tool_result = await maybe_await(
                            event_caller(
                                {
                                    'type': 'execute:tool',
                                    'data': {
                        ...
                        tool_result = await maybe_await(tool_function(**tool_function_params))
